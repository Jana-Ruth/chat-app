  ...
  io.on('connection', async (socket) => {
    ...
    socket.on('message:send', async ({ conversationId, text, attachment }, ack) => {
      try {
        ...
        if (!hasText && !hasAttachment) {
          return ack?.({ error: 'Message must have text or an attachment' });
        }
    ...
    socket.on('call:invite', async ({ toUserId, conversationId, callType, sdp }, ack) => {
      try {
        ...
        const [me, other] = await Promise.all([
          User.findById(userId).select('username avatarUrl blockedUsers'),
        ...
        io.to(`user:${toUserId}`).emit('call:incoming', {
          ...
          callType,
          sdp,
          fromUser: { id: me._id, username: me.username, avatarUrl: me.avatarUrl },
        });
        ack?.({ callId });
      } catch (err) {
