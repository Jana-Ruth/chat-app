...
export default function ChatWindow({ conversation, onConversationUpdated, onLeftGroup }) {
  ...
  async function sendMessage({ attachment } = {}) {
    ...
    socket.emit('message:send', { conversationId: conversation._id, text: trimmed, attachment }, (res) => {
    ...
    <div className="chat-window" style={backgroundStyle(background)}>
      <div className="chat-window-header">
        {conversation.isGroup ? (
          <button className="chat-header-title" onClick={() => setShowGroupPanel(true)}>
            <span className="avatar" style={{ width: 34, height: 34, fontSize: 14 }}>#</span>
            {conversation.name}
          </button>
        ) : (
        ...
        <div className="chat-header-actions">
          ...
          <button className="icon-toggle" title="Search messages" onClick={() => setShowSearch(true)}>🔍</button>
          <button className="icon-toggle" title="Chat background" onClick={() => setShowBgPicker(true)}>🎨</button>
        </div>
      </div>
                ...
                {isMine && !m.deleted && !isEditing && (
                  <div className="message-actions">
                    {m.text && !m.attachment?.url && (
                      <button onClick={() => startEdit(m)} title="Edit">✏️</button>
                    )}
                    <button onClick={() => handleDelete(m._id)} title="Delete">🗑️</button>
                  </div>
                )}
              </div>
            </div>
      ...
      {pendingUpload && (
        <div className="pending-upload">
          ...
          <button onClick={() => setPendingUpload(null)}>✕ Remove</button>
        </div>
      )}
      ...
      <form className="message-input" onSubmit={handleSend}>
        ...
        <button
          ...
          disabled={uploading}
        >
          {voiceRecorder.recording ? `⏹ ${voiceRecorder.seconds}s` : '🎤'}
        </button>
