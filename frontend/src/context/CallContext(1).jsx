...
export function CallProvider({ children }) {
  ...
  const endCall = useCallback(
    (opts = {}) => {
      ...
      if (info && notifyRemote && socket) {
        socket.emit('call:end', { callId: info.callId, toUserId: info.otherUser.id });
      }
  ...
  const startCall = useCallback(
    async (conversationId, otherUser, callType) => {
      ...
      try {
        ...
        socket.emit(
          'call:invite',
          { toUserId: otherUser.id, conversationId, callType, sdp: offer },
          (res) => {
            if (res?.error) {
              setError(res.error);
              cleanup();
  ...
  const acceptCall = useCallback(async () => {
    ...
    try {
      ...
      const pc = createPeerConnection(info.otherUser.id);
      stream.getTracks().forEach((track) => pc.addTrack(track, stream));
      ...
      socket.emit('call:accept', { callId: info.callId, toUserId: info.otherUser.id, sdp: answer });
      setCallState('active');
      startTimer();
    } catch (err) {
      console.error('acceptCall error:', err);
      setError('Could not access camera/microphone');
      socket.emit('call:decline', { callId: info.callId, toUserId: info.otherUser.id });
      cleanup();
      setCallState('idle');
      setCallInfo(null);
    }
  ...
  const cancelCall = useCallback(() => {
    const info = callInfoRef.current;
    if (info && socket) {
      socket.emit('call:cancel', { callId: info.callId, toUserId: info.otherUser.id });
    }
    logCall('missed', 0);
    cleanup();
    setCallState('idle');
  ...
  useEffect(() => {
    ...
    function handleIncoming({ callId, conversationId, callType, sdp, fromUser }) {
      ...
      if (callInfoRef.current) {
        socket.emit('call:decline', { callId, toUserId: fromUser.id });
        return;
      }
